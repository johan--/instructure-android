...
public abstract class BaseLoginActivity extends BaseParentActivity {
    ...
    private void setupCallbacks() {
        mGetToken = new StatusCallback<OAuthToken>(){
            @Override
            public void onResponse(retrofit2.Response<OAuthToken> response, LinkHeaders linkHeaders, ApiType type) {
                ...
                ApiPrefs.setToken(response.body().getAccessToken());
            }
    ...
    protected String createUserAgent(String userAgentString) {
        ...
        ApiPrefs.setUserAgent(userAgent);
        return userAgent;
    }
    ...
    private void loadAuthenticatedURL() {
        ...
        mAuthenticationURL = "https://" + ApiPrefs.getDomain() + "/login/oauth2/auth?client_id=" +
        ...
        if (canvas_login == 1) {
            ...
            CookieManager cookieManager = CookieManager.getInstance();
            cookieManager.setCookie("http://" + ApiPrefs.getDomain(), "canvas_sa_delegated=1");
        }
