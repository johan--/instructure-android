...
public class NoStudentsPage extends BasePage {

    //region UI Element Locator Methods

    private ViewInteraction addStudentIcon() {
        return onView(withId(R.id.addStudentIcon));
    }

    private ViewInteraction noStudentsText() {
    ...
    private ViewInteraction logoutButton() {
        return onView(withId(R.id.log_out));
    }
    ...
    public void assertPageObjects() {
        addStudentIcon().check(matches(isDisplayed()));
        noStudentsText().check(matches(isDisplayed()));
