...
class PollSessionUnitTest : Assert() {
    ...
    fun testPollSession() {
        ...
        for (pollSession in pollSessions) {
            ...
            Assert.assertNotNull(pollSession.created_at)

            Assert.assertTrue(pollSession.id > 0)

            Assert.assertTrue(pollSession.course_id > 0)
            Assert.assertTrue(pollSession.course_section_id > 0)
            Assert.assertTrue(pollSession.poll_id > 0)
        }
    }
