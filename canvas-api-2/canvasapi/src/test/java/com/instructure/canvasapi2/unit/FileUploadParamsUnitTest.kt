...
class FileUploadParamsUnitTest : Assert() {
    ...
    fun testDiscussionTopic() {
        ...
        Assert.assertEquals(paramsHash.get("AWSAccessKeyId"), "ASDFDDFKJDSF")
        Assert.assertEquals(paramsHash.get("key"), "account_99298/attachments/67193665/IMG_FILENAME.jpg")
        Assert.assertEquals(paramsHash.get("acl"), "private")
        Assert.assertEquals(paramsHash.get("Policy"), "eyJleHBpcmF0aW9uIjoiMjAxT2xyNlYifSx7ImNvbnRlbnQtdHlwZSI6ImltYWdlXC9qcGV")
        Assert.assertEquals(paramsHash.get("Signature"), "BNudkjo4pLUIUHYYnbT0D8phmBw")
        Assert.assertEquals(paramsHash.get("success_action_redirect"), "https://mobiledev.instructure.com/api/v1/files/67193665/create_success?uuid=dssdfsfdsfs")
        Assert.assertEquals(paramsHash.get("content-type"), "image/jpeg")
    }
