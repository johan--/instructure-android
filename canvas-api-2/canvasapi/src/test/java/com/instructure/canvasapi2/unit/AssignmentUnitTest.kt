...
class AssignmentUnitTest : Assert() {
    ...
    fun testAssignment() {
        ...
        Assert.assertEquals(assignment.pointsPossible, 30.0)

        Assert.assertEquals(assignment.submissionTypes.size, 3)
        Assert.assertTrue(assignment.submissionTypes[0].toString().equals("online_upload", ignoreCase = true))
        Assert.assertTrue(assignment.submissionTypes[1].toString().equals("online_text_entry", ignoreCase = true))
        Assert.assertTrue(assignment.submissionTypes[2].toString().equals("media_recording", ignoreCase = true))
    ...
    @Language("JSON")
    private var assignmentDueDates = """
      {
        "assignment_group_id": 534100,
        "automatic_peer_reviews": false,
        "description": "<p>List all the different types of layouts that are used in xml.</p>",
