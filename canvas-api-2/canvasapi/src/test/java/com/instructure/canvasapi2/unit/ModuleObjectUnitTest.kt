...
class ModuleObjectUnitTest : Assert() {
    ...
    fun testModuleObject() {
        ...
        for (i in moduleObjects.indices) {
            ...
            if (i == 1) {
                Assert.assertNotNull(moduleObjects[i].unlock_at)
            }
            ...
            if (i > 0) {
                for (j in 0 until moduleObjects[i].prerequisite_ids.size) {
                    Assert.assertTrue(moduleObjects[i].prerequisite_ids[j] > 0)
                }
            }
        }
    }
