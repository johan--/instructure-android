...
public class PollsChoiceAPI {
    ...
    interface PollChoiceInterface {
        ...
        Call<PollChoiceResponse> getNextPagePollChoicesList(
                @Path(value = "next", encoded = false) String nextURL);

        @GET("polls/{pollId}/poll_choices/{poll_choice_id}")
        Call<PollChoiceResponse> getSinglePollChoice(
                @Path("pollId") long pollId,
    ...
    public static void getSinglePollChoice(long pollId, long pollChoiceId, RestBuilder adapter, RestParams params, StatusCallback<PollChoiceResponse> callback) {
        callback.addCall(adapter.build(PollChoiceInterface.class, params).getSinglePollChoice(pollId, pollChoiceId)).enqueue(callback);
    }
