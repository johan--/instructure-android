...
public class DepaginatedCallback<MODEL extends CanvasModel> extends StatusCallback<List<MODEL>> {
    ...
    @Override
    public void onResponse(Response<List<MODEL>> response, LinkHeaders linkHeaders, ApiType type, int code) {
        ...
        if(type.isCache()) {
            ...
            if (StatusCallback.moreCallsExist(linkHeaders)) {
                ...
                callback.onResponse(Response.success(cacheItems, response.raw()), linkHeaders, ApiType.CACHE, code);
            }
        } else {
            ...
            if (StatusCallback.moreCallsExist(linkHeaders)) {
                ...
                callback.onResponse(Response.success(networkItems, response.raw()), linkHeaders, ApiType.API, code);
            }
        }
    }

    @Override
    public void onFail(Call<List<MODEL>> callResponse, Throwable error, Response response) {
        callback.onFail(callResponse, error, response);
    }
}
