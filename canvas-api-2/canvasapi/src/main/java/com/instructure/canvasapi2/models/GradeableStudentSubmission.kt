...
data class GradeableStudentSubmission(
...
) : CanvasModel<GradeableStudentSubmission>(), PaperParcelable {

    override fun getId() = when (assignee) {
    ...
    override fun getComparisonDate() = submission?.comparisonDate ?: when (assignee) {
        is StudentAssignee -> assignee.student.comparisonDate
        is GroupAssignee -> assignee.group.comparisonDate
    }

    override fun getComparisonString() = submission?.comparisonString ?: when (assignee) {
    ...
    companion object {
        ...
        @JvmField val CREATOR = PaperParcelGradeableStudentSubmission.CREATOR
    }
}
