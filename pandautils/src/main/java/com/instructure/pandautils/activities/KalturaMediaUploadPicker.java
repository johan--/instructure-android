...
public class KalturaMediaUploadPicker extends Activity {
    ...
    private void chooseMedia() {
        if(PermissionUtils.hasPermissions(KalturaMediaUploadPicker.this, PermissionUtils.WRITE_EXTERNAL_STORAGE)) {
            Intent intent = new Intent(Intent.ACTION_GET_CONTENT, android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
            //only let the user use local images (no picasa...)
            intent.putExtra(Intent.EXTRA_LOCAL_ONLY, true);
            intent.setType("video/*, audio/*");
            File file = new File(getFilesDir(), "/image/*");
            intent.setDataAndType(getUriForFile(KalturaMediaUploadPicker.this, KalturaMediaUploadPicker.this.getApplicationContext().getPackageName() + Const.FILE_PROVIDER_AUTHORITY, file), "video/*");
    ...
    private Uri getOutputMediaFileUri() {
        ...
        //set the file uri for use with Kaltura uploading. Kaltura doesn't deal with content:// uris
        capturedImageURI = Uri.parse(mediaFile.getAbsolutePath());

        //return the content uri for starting the intent
        return getUriForFile(KalturaMediaUploadPicker.this, KalturaMediaUploadPicker.this.getApplicationContext().getPackageName() + Const.FILE_PROVIDER_AUTHORITY, mediaFile);
    }
