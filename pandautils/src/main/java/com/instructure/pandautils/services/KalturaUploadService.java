...
public class KalturaUploadService extends Service implements APIStatusDelegate {
        ...
        fileUpload = new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Response response = KalturaAPI.uploadKalturaFile(getContext(), kalturaToken, kalturaUploadToken, typedFile, fullUrl);
                    if (response != null && response.getStatus() == 201) {
                        callNextAPIcall(response, KalturaAPI.getMediaIdForUploadedFileTokenSynchronous(getContext(), kalturaToken, kalturaUploadToken, typedFile.fileName(), typedFile.mimeType()));
                    } else {
                        uploadError();
                    }
        ...
        kalturaConfigCallback = new CanvasCallback<KalturaConfig>(this, errorDelegate) {
            ...
            public void firstPage(KalturaConfig kalturaConfig, LinkHeaders linkHeaders, Response response) {
                if (kalturaConfig.isEnabled()) {
                    ...
                    uploadError(kalturaConfig, response);
                }
        ...
        kalturaSessionCallback = new CanvasCallback<KalturaSession>(this, errorDelegate) {
            ...
            public void firstPage(KalturaSession kalturaSession, LinkHeaders linkHeaders, Response response) {
                if (!KalturaRestAdapter.setupInstance(getContext(), kalturaSession.getKs(), ksDomain)) {
                    uploadError(kalturaSession, response);
                }
    ...
    private void uploadError(Exception e) {
        Logger.e("KALTURA EXCEPTION: " + e);
        showErrorNotification();
    }

    private void uploadError(RetrofitError e) {
        Response response = e.getResponse();
        RetrofitError.Kind kind = e.getKind();
        Logger.e("KALTURA RETROFIT ERROR: " + e + " MESSAGE: " + e.getMessage() + " BODY: " + e.getBody());
        if(kind != null) {
            Logger.e("-- KIND: " + kind.toString());
        }
        if(response != null) {
            Logger.e("-- RESPONSE: " + response.getStatus() + " REASON: " + response.getReason());
        }
        showErrorNotification();
    }

    private void uploadError(xml xmlModel, Response response) {
        Logger.e("KALTURA XML/RESPONSE ERROR: " + xmlModel.toString() + " RESPONSE: " + response.getReason() + " CODE: " + response.getStatus());
        showErrorNotification();
    }

    private void uploadError(Response response) {
        Logger.e("KALTURA RESPONSE ERROR: " + response.getReason() + " CODE: " + response.getStatus());

        showErrorNotification();
    }

    private void uploadError(KalturaSession session, Response response) {
        Logger.e("KALTURA SESSION/RESPONSE ERROR: " + session.toString() + " REASON: " + response.getReason() + " CODE: " + response.getStatus());
        showErrorNotification();
    }

    private void uploadError(KalturaConfig config, Response response) {
        Logger.e("KALTURA CONFIG/RESPONSE ERROR: " + config.toString() + " REASON: " + response.getReason() + " CODE: " + response.getStatus());
    ...
    private void callNextAPIcall(Response response, xml xml) {
        ...
        if (ACTION.isSubmissionComment(action)) {
            SubmissionAPI.postMediaSubmissionComment(course, assignmentId, studentId, mediaId, mediaType, isGroupComment, submissionCanvasCallback);
        }
        else if(ACTION.isAssignmentSubmission(action)){
