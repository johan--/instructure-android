...
public class FileUploadService extends IntentService implements APIStatusDelegate{
    ...
    public static final String ACTION_NEW_MESSAGE_ATTACHMENTS   = "ACTION_NEW_MESSAGE_ATTACHMENTS";
    ...
    private void startUploads(String action, ArrayList<FileSubmitObject> fileSubmitObjects, Bundle bundle){
        ...
        final String subject      = bundle.getString(Const.SUBJECT);
        final boolean isGroup     = bundle.getBoolean(Const.IS_GROUP);
        final ArrayList<String> userIds = bundle.getStringArrayList(Const.USER_IDS);
        final String contextId    = bundle.getString(Const.CONTEXT_ID);
        ...
        try{
            for(int i = 0; i < fileSubmitObjects.size(); i++){
                ...
                else if(ACTION_MESSAGE_ATTACHMENTS.equals(action) || ACTION_NEW_MESSAGE_ATTACHMENTS.equals(action)){
            ...
            } else if(ACTION_NEW_MESSAGE_ATTACHMENTS.equals(action)) {
                submitAttachmentsForNewMessage(messageText, subject, userIds, contextId, isGroup, attachmentsIds);
            } else{
    ...
    private void submitAttachmentsForNewMessage(String messageText, String subject, ArrayList<String> userIds, String contextId, boolean isGroup, ArrayList<String> attachmentsIds){
        Response response = null;
        try{
            response = ConversationAPI.createConversationWithAttachmentSynchronous(this, userIds, messageText, subject, contextId, isGroup, attachmentsIds);
        }catch(RetrofitError error){
            handleRetrofitError(error);
            broadcastError(getString(R.string.errorSendingMessage));
        }
    ...
    private void broadCastNewMessageSuccess() {
        final Intent status = new Intent(ALL_UPLOADS_COMPLETED);
        sendBroadcast(status);
    }
    ...
    public static Bundle getNewMessageBundle(ArrayList<FileSubmitObject> fileSubmitObjects, ArrayList<String> userIds, String subject, String messageText, boolean isGroup, String contextId){
        //Long size, String fileName, String contentType, String path
        Bundle bundle = new Bundle();
        bundle.putParcelableArrayList(Const.FILES, fileSubmitObjects);
        bundle.putStringArrayList(Const.USER_IDS, userIds);
        bundle.putString(Const.SUBJECT, subject);
        bundle.putString(Const.MESSAGE, messageText);
        bundle.putBoolean(Const.IS_GROUP, isGroup);
        bundle.putString(Const.CONTEXT_ID, contextId);
