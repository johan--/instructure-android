...
public abstract class BaseDiscussionDetailsFragment extends BaseFragment implements
    ...
    public void onCreateView(View view) {
        ...
        mVideoViewClient = new ActivityContentVideoViewClient(getActivity(), new ActivityContentVideoViewClient.HostingView() {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (i >= 0 && i < getCurrentEntry().getReplies().size()) {
                        ...
                        DiscussionManager.rateDiscussionEntry(getActiveCanvasContext(), getHeader().getId(), entry.getId(), rating, new StatusCallback<Void>(){
    ...
    private void addSentDiscussionEntry(final DiscussionEntry discussionEntry) {
        if (!getTopic().isForbidden()) {
            ...
            final String colorString = CanvasContextColor.getColorStringFromInt(ThemePrefs.getAccentColor(), true);
    ...
    private void getFullDiscussionTopic() {
        DiscussionManager.getFullDiscussionTopic(getActiveCanvasContext(), mDiscussionTopicHeader.getId(), new StatusCallback<DiscussionTopic>() {
