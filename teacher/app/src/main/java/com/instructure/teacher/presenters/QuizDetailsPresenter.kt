    ...
    override fun loadData(forceNetwork: Boolean) {
        ...
        mApiCalls = weave {
            ...
            try {
                ...
                if (mQuiz.quizType == "assignment" && mQuiz.assignment == null && mQuiz.assignmentId > 0) {
                    //now get the assignment
                    val assignment = awaitApi<Assignment> { AssignmentManager.getAssignment(mQuiz.assignmentId, mCourse.id, forceNetwork, it) }
                    mQuiz.assignment = assignment
                }

                if (mQuiz.assignmentGroup == null && mQuiz.assignmentGroupId != 0L) {
                    mQuiz.assignmentGroup = awaitApi<AssignmentGroup> {
