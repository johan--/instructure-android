...
class DiscussionListAdapter(context: Context,
    ...
    override fun createViewHolder(v: View, viewType: Int): RecyclerView.ViewHolder {
        when (viewType) {
            Types.TYPE_ITEM -> return DiscussionListHolder(v)
            else -> if(mIsAnnouncement) return EmptyViewHolder(v) else return DiscussionExpandableViewHolder(v)
        }
    }

    override fun itemLayoutResId(viewType: Int): Int {
        when (viewType) {
            Types.TYPE_ITEM -> return DiscussionListHolder.holderResId
            else -> if(mIsAnnouncement) return EmptyViewHolder.holderResId else return DiscussionExpandableViewHolder.holderResId
        }
    }
    ...
    override fun onBindHeaderHolder(holder: RecyclerView.ViewHolder?, group: String, isExpanded: Boolean) {
        if(!mIsAnnouncement) {
            context?.let {
                (holder as DiscussionExpandableViewHolder).bind(isExpanded, holder, group, {
                    discussionGroup ->
