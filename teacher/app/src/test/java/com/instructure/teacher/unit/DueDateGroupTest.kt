...
class DueDateGroupTest {
    ...
    lateinit var postData: AssignmentPostBody
    lateinit var assignmentWithoutOverrides: Assignment
    lateinit var assignmentWithSimilarOverrides: Assignment
    lateinit var assignmentWithDifferentOverrides: Assignment
    lateinit var assignmentWithoutBaseDate: Assignment

    private val randy = Random()

    val sampleDateStrings = Array(15) { Date(Math.abs(randy.nextLong())).toApiString()!! }

    val sampleDates = sampleDateStrings.map { APIHelper.stringToDate(it) }

    @Before
    fun setup() {
        postData = AssignmentPostBody()

        assignmentWithoutOverrides = Assignment().apply {
            setDueAt(sampleDateStrings[0])
            setLockAt(sampleDateStrings[0])
            setUnlockAt(sampleDateStrings[0])
            allDates = listOf(AssignmentDueDate().apply {
                isBase = true
                setDueAt(sampleDateStrings[0])
                setLockAt(sampleDateStrings[0])
                setUnlockAt(sampleDateStrings[0])
        ...
        assignmentWithSimilarOverrides = Assignment().apply {
            setDueAt(sampleDateStrings[0])
            setLockAt(sampleDateStrings[0])
            setUnlockAt(sampleDateStrings[0])

            overrides = (1..3).map {
                AssignmentOverride().apply {
                    id = it.toLong()
                    setDueAt(sampleDates[0])
                    setLockAt(sampleDates[0])
                    setUnlockAt(sampleDates[0])
                    when (it) {
                        1 -> courseSectionId = Math.abs(randy.nextLong())
                        2 -> groupId = Math.abs(randy.nextLong())
                        3 -> studentIds = LongArray(randy.nextInt(5) + 1) { Math.abs(randy.nextLong()) }
                    }
                }
            }

            val allDueDates = arrayListOf(AssignmentDueDate().apply {
                isBase = true
                setDueAt(sampleDateStrings[0])
                setLockAt(sampleDateStrings[0])
                setUnlockAt(sampleDateStrings[0])
            ...
            allDueDates += (1..3).map { AssignmentDueDate().apply {
                id = it.toLong()
                setDueAt(sampleDateStrings[0])
                setLockAt(sampleDateStrings[0])
                setUnlockAt(sampleDateStrings[0])
        ...
        assignmentWithoutBaseDate = Assignment().apply {
            ...
            allDates = (1..3).map { AssignmentDueDate().apply {
                id = it.toLong()
                setDueAt(sampleDateStrings[0])
                setLockAt(sampleDateStrings[0])
                setUnlockAt(sampleDateStrings[0])
        ...
        assignmentWithDifferentOverrides = Assignment().apply {
            setDueAt(sampleDateStrings[0])
            setLockAt(sampleDateStrings[0])
            setUnlockAt(sampleDateStrings[0])
            ...
            overrides = (1 until sampleDateStrings.size).map {
                AssignmentOverride().apply {
                    id = it.toLong()
                    setDueAt(sampleDates[it])
                    setLockAt(sampleDates[it])
                    setUnlockAt(sampleDates[it])
                    when (it % 3) {
                        0 -> courseSectionId = Math.abs(randy.nextLong())
                        1 -> groupId = Math.abs(randy.nextLong())
                        2 -> studentIds = LongArray(randy.nextInt(5) + 1) { Math.abs(randy.nextLong()) }
                    }
                }
            }

            allDates = (0 until sampleDateStrings.size).map { AssignmentDueDate().apply {
                if (it == 0) isBase = true else id = it.toLong()
                setDueAt(sampleDateStrings[it])
                setLockAt(sampleDateStrings[it])
                setUnlockAt(sampleDateStrings[it])
            } }
        }
    }

    @Test fun `Has correct date contents for assignment without overrides`() {
    ...
    @Test fun `Has correct date contents for assignment with similar overrides`() {
    ...
    @Test fun `Has correct date contents for assignment without base date`() {
    ...
    @Test fun `Has correct date contents for assignment with different overrides`() {
    ...
    @Test fun `Correctly saves dates for assignment without overrides`() {
    ...
    @Test fun `Correctly saves dates for assignment with similar overrides`() {
    ...
    @Test fun `Correctly saves dates for assignment without base date`() {
    ...
    @Test fun `Correctly saves dates for assignment different overrides`() {
    ...
    private fun assertUpdatedForSave(original: Assignment, saved: AssignmentPostBody) {
        ...
        val originalStudents = original.overrides?.flatMap { it.studentIds?.asList() ?: emptyList() }?.sorted() ?: emptyList()
